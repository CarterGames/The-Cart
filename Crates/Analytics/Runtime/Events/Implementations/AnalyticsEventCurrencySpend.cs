...
{
    public class AnalyticsEventCurrencySpend : AnalyticsEvent
    {
        public override string EventName => "currency_spend";
...
#if CARTERGAMES_CART_CRATE_CURRENCY
        public AnalyticsEventCurrencySpend(CurrencyAccount account, AccountTransaction transaction, params string[] virtualCurrencyCategories)
        {
            CurrencyName = account.Id;
            CurrencyValue = transaction.StartingValue - transaction.NewValue;
            TotalCurrency = transaction.NewValue;
            VcCategories = virtualCurrencyCategories;
        }
#endif
        
        public AnalyticsEventCurrencySpend(string name, double amount, double total, params string[] virtualCurrencyCategories)
        {
            CurrencyName = name;
