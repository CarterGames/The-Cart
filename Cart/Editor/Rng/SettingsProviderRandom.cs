    ...
    public class SettingsProviderRandom : ISettingsProvider
    {
        ...
        public void OnProjectSettingsGUI()
        {
            ...
            void OnSelectionMade(SearchTreeEntry entry)
            {
                ...
                if (entry.userData.GetType().GetInterfaces().All(t => t != (typeof(ISeededRngProvider)))) return;

                var seededProvider = AssetRef.RngProviderAssemblyClassDef.GetTypeInstance<ISeededRngProvider>();
                ObjectRef.Fp("editorOverrideSeed").stringValue = seededProvider.GenerateSeed();
                ObjectRef.ApplyModifiedProperties();
                ObjectRef.Update();
            }
            ...
            EditorGUILayout.PropertyField(ObjectRef.Fp("useOverrideSeed"));
            ...
            if (ObjectRef.Fp("useOverrideSeed").boolValue)
            {
                EditorGUILayout.BeginHorizontal();
                
                EditorGUILayout.PropertyField(ObjectRef.Fp("editorOverrideSeed"));
                
                // Draws the button to copy the seed...
                if (GUILayout.Button("Copy", GUILayout.Width(65)))
                {
                    ObjectRef.Fp("editorOverrideSeed").stringValue.CopyToClipboard();
